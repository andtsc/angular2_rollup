...
const copy = {
    public: (path) => {
        ...
        log(path || 'src/public/', 'copied', 'to', paths.fsrun+'/');
    ...
    file: (path) => {
        cp('-R', path, paths.fsrun+'/');
        log(path, 'copied', 'to', paths.fsrun+'/');
    },
    html: (path) => {
        ls('src/app/**/*.html').forEach(function(file) {
          cp(file, paths.fsrun+'/'+file);
          log(file.replace(/^.*[\\\/]/, ''), 'copied', 'to',  paths.fsrun+'/'+file.substring(0, file.lastIndexOf("/")));
        });
    },
    lib: () => {

        mkdir('-p', __dirname + '/' + paths.fslib);

        for( var i=0;  i < paths.dep.length; i++ ) {

            cp('-R', paths.src + '/' + paths.dep[i], paths.fslib + '/' + paths.dep[i]);
            log(paths.dep[i], 'copied', 'to',  paths.fslib + '/' + paths.dep[i]);
...
const compile = {
    ...
    clean: (path) => {
      ...
      const outFile = path ? path : './'+paths.fsrun+'/bundle.js';
        ...
        let clean = exec(scripts['clean:ngfactory'], function(code, output, error) {
              ...
              let tsc = exec(scripts['build:ngc'], function(code, output, error) {
                 ...
                 let bundle = exec(scripts['bundle:src'], function(code, output, error) {
                     log('Rollup', 'bundled', 'bundle.es2015.js in', './'+paths.fsrun);
                     ...
                     let closure = exec(scripts['transpile:closure'], function(code, output, error){
                          log('Closure Compiler', 'transpiled', './'+paths.fsrun+'/bundle.es2015.js to', './'+paths.fsrun+'/bundle.js');
    ...
    ts : (path) => {
        ...
        let clean = exec(scripts['clean:ngfactory'], function(code, output, error) {
            ...
            let tsc = exec(scripts['transpile:src'], function(code, output, error) {
                if (path) {
                  log('typescript', 'transpiled', path+' to', paths.fsrun+'/'+path.replace('.ts','.js'));
                  cp(path, paths.fsrun+'/'+path);
                } else {
                  log('typescript', 'transpiled', 'src/*ts to', paths.fsrun+'/src/*ts');
                }
...
let style = {
    ...
    file: (path, watch) => {
        ...
        let outFile = path.indexOf('src/style') > -1 ? paths.fsrun+'/style/style.css' : (env === 'dev') ? paths.fsrun+'/'+srcPath+'/'+filename.replace('.scss','.css') : path.replace('.scss','.css').replace('src', 'tmp');
        sass.render({
          file: path,
          outFile: outFile,
